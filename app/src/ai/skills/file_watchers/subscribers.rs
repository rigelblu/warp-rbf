...
impl RepositorySubscriber for HomeSkillSubscriber {
    fn on_scan(
        ...
        // Scan from the original (user-facing) provider path — not the
        // repository's canonical root, which won't prefix-match the un-resolved
        // home_skills_path() candidates when the provider parent is a symlink.
