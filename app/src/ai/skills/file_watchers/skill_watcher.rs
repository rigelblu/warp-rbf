...
pub struct SkillWatcher {
    ...
    /// originals when providers share a target; events fan out to each. When
    /// the canonical dir is itself a registered provider (`~/.claude` →
    /// `~/.agents`), the identity path joins the fan-out.
    ///
    /// TRANSITIONAL: this is the parent-layer twin of `symlink_canonical_to_originals`
...
impl SkillWatcher {
    ...
    /// pointing at a shared dir), so we fan out to all of them. When the matched
    /// canonical is itself a registered provider, the identity path is kept
    /// alongside the originals. Returns `path` unchanged in a single-element vec
    /// if it doesn't match any known canonical.
    fn translate_canonical_to_original_paths(&self, path: &Path) -> Vec<PathBuf> {
        ...
        match best {
            Some((canonical, originals, rel)) => {
                let mut translated: Vec<PathBuf> =
                    originals.iter().map(|orig| orig.join(rel)).collect();
                // The canonical dir can itself be a registered provider (e.g.
                // `~/.claude` → `~/.agents`): the identity path is then a real
                // provider's own path, not a resolution artifact — dropping it
                // would starve that provider of its events.
                //
                // Keyed by `canonical` though `home_provider_watchers` is keyed by
                // *original* paths — correct, not a type confusion: the identity
                // push is needed only when the canonical dir is a provider
                // registered under its own name (a non-symlinked provider, whose
                // original == canonical), so it's a key exactly when this must fire.
                if self.home_provider_watchers.contains_key(canonical) {
                    translated.push(path.to_path_buf());
                }
                log::debug!(
                    "skill_watcher: translated canonical path {} -> {:?}",
                    path.display(),
                    translated
                );
                translated
    ...
    fn rollback_home_provider_watch_registration(
        &mut self,
        path: &Path,
        repo_handle: &ModelHandle<Repository>,
        subscriber_id: SubscriberId,
        ctx: &mut ModelContext<Self>,
    ) {
        let registration_is_current = self.home_provider_watchers.get(path).is_some_and(
            |(current_repo_handle, current_subscriber_id)| {
                current_repo_handle == repo_handle && *current_subscriber_id == subscriber_id
            },
        );

        if registration_is_current {
            self.home_provider_watchers.remove(path);
            Self::remove_home_provider_original_mapping(
                &mut self.home_provider_canonical_to_originals,
                path,
            );
    ...
    /// committed watches. Re-registering the same original (symlink retarget)
    /// purges its previous mapping up front, so the map tracks only the live
    /// target.
    fn watch_home_provider_path(
        ...
        // A retarget (`ln -sfn new-target ~/.agents`) re-registers the same
        // original with a new canonical; purge the previous mapping or the
        // stale entry keeps translating events from the old target into
        // phantom provider paths. Runs ahead of the canonicalize guard so a
        // retarget to a broken target still clears it. No-op for fresh
        // registrations; other originals' entries are untouched.
        Self::remove_home_provider_original_mapping(home_provider_canonical_to_originals, path);
        ...
        if let Some((old_repo_handle, old_subscriber_id)) =
            home_provider_watchers.insert(path.to_path_buf(), (repo_handle.clone(), subscriber_id))
        {
            old_repo_handle.update(ctx, |repo, ctx| {
                repo.stop_watching(old_subscriber_id, ctx);
            });
        }
        ...
        ctx.spawn(start.registration_future, move |me, res, ctx| {
            if let Err(err) = res {
                ...
                me.rollback_home_provider_watch_registration(
                    &path_owned,
                    &repo_handle,
                    subscriber_id,
                    ctx,
