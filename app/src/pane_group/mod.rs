...
pub struct EditorTabSplitHintModel {
    target: Option<EditorTabSplitHint>,
}

#[derive(Clone, Copy, PartialEq, Eq)]
pub struct EditorTabSplitHint {
    pub pane: PaneId,
    pub direction: Direction,
}

impl EditorTabSplitHintModel {
    /// The pane and region that should currently show the "release to split here" hint, if any.
    pub fn target(&self) -> Option<EditorTabSplitHint> {
        self.target
    }
}
...
impl PaneGroup {
        ...
        if self.pane_contents.contains_key(&pane_id) {
            match event {
                PaneViewEvent::MovePaneWithinPaneGroup {
                    ...
                    match origin {
                        ...
                        ActionOrigin::EditorTab(_) => {
                            ...
                            self.set_editor_tab_split_hint(
                                Some(EditorTabSplitHint {
                                    pane: *target_id,
                                    direction: *direction,
                                }),
                                ctx,
                ...
                PaneViewEvent::PaneDraggedOutsideTabBarOrPaneGroup {
                    ...
                    if matches!(origin, ActionOrigin::EditorTab(_)) {
                        // warp-44: an editor tab can leave pane drop targets and still be a valid
                        // ungroup. Resolve the pane under the drag position ourselves because
                        // editor-tab drags do not always deliver `PaneDropTargetData` for other
                        // panes. The stored hint is also the eventual split target on drop.
                        let hint = self.editor_tab_split_hint_for_drag_position(
                            pane_id,
                            *drag_position,
                            ctx,
                        );
                        self.set_editor_tab_split_hint(hint, ctx);
                    } else {
                        self.set_editor_tab_split_hint(None, ctx);
                    }
                ...
                PaneViewEvent::PaneDroppedOutsideTabBarOrPaneGroup { origin } => {
                    ...
                    let split = if let ActionOrigin::EditorTab(editor_tab_index) = origin {
                        let hint = EditorTabSplitHintModel::as_ref(ctx).target();
                        let target_pane_id = hint.map(|hint| hint.pane).unwrap_or(pane_id);
                        let direction = hint.map(|hint| hint.direction).unwrap_or(Direction::Right);
                        self.set_editor_tab_split_hint(None, ctx);
                ...
                PaneViewEvent::DroppedWithinPaneGroup {
                    ...
                    ActionOrigin::EditorTab(editor_tab_index) => {
                        ...
                        let direction = self
                            .editor_tab_split_hint_direction_for_pane(*target_id, ctx)
    ...
    fn editor_tab_split_hint_direction_for_pane(
        &self,
        pane_id: PaneId,
        ctx: &AppContext,
    ) -> Option<Direction> {
        EditorTabSplitHintModel::as_ref(ctx)
            .target()
            .filter(|hint| hint.pane == pane_id)
            .map(|hint| hint.direction)
    }

    fn editor_tab_split_hint_for_drag_position(
        &self,
        source_pane_id: PaneId,
        drag_position: RectF,
        ctx: &mut ViewContext<Self>,
    ) -> Option<EditorTabSplitHint> {
        let drag_center = drag_position.center();

        let target = self.panes.visible_pane_ids().into_iter().find_map(|pane| {
            let rect = ctx.element_position_by_id(pane.position_id())?;
            rect.contains_point(drag_center).then_some((pane, rect))
        });

        let (pane, rect) = target.or_else(|| {
            ctx.element_position_by_id(source_pane_id.position_id())
                .map(|rect| (source_pane_id, rect))
        })?;

        Some(EditorTabSplitHint {
            pane,
            direction: calculate_pane_move_direction(rect, drag_position)
                .unwrap_or(Direction::Right),
        })
    }
    ...
    pub(crate) fn split_editor_tab_into_code_pane(
        ...
        match self.try_add_pane_with_options(
            ...
            Ok(new_pane_id) => {
                ...
                log::debug!(
                    "[warp-44 ungroup] split editor tab into new pane: source={source_pane_id:?} tab={editor_tab_index} target={target_pane_id:?} direction={direction:?} new_pane={new_pane_id:?}"
                );
                true
            }
            Err(rejected_pane) => {
