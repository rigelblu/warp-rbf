...
#[test]
fn test_accessibility_data_prefers_input_selected_text() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        ...
        terminal.update(&mut app, |view, ctx| {
            view.focus_terminal(ctx);
            view.typed_characters_on_terminal("prompt selection", ctx);
            view.input.update(ctx, |input, ctx| {
                input
                    .editor()
            ...
            let selected_text = view
                .accessibility_data(ctx)
                .and_then(|data| data.selected_text);
            assert_eq!(selected_text.as_deref(), Some("prompt selection"));
            ...
            let terminal_selected_text = view.selected_text(ctx).filter(|text| !text.is_empty());
            assert!(
                terminal_selected_text.is_some(),
                "expected an active terminal output selection"
            );
